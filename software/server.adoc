...
dass man seinen Gegner nicht mit illegalen Nachrichten belästigen kann,
...
NOTE: Der TestClient kann sich auch mit einem bereits laufenden Server verbinden, wie bei der Variante ohne TestClient,
...
Die Ergebnisse der Spiele werden für den jeweiligen Spielernamen vom Server zusammengezählt, auch über mehrere Starts des TestClients.
Die Ergebnisse werden erst zurückgesetzt, wenn der Server neu gestartet wird.
Achte also nach einer Veränderung der Spieler darauf, den Server neuzustarten oder andere Spielernamen zu verwenden.
...
Nun können jederzeit die Testdaten der Spieler anhand ihres Anzeigenamens erfragt werden:
