...
image::wettkampfsystem_startseite.jpg[Die Startseite der Weboberfläche]
...
Hier sind alle News zum Wettkampf einsehbar.
Die neusten News werden auch immer oben neben dem Logo angezeigt.
Alles Wichtige zum Wettkampf erscheint hier.
...
image::wettkampfsystem_spieltaguebersicht.jpg[]
...
image::wettkampfsystem_rangliste.jpg[Screenshot der Rangliste]
...
image::wettkampfsystem_teamtabelle.jpg[Die Teamtabelle]
...
image::wettkampfsystem_neue_person.jpg[Dialog, um neue Personen ins Team aufzunehmen]
...
Ganz unten auf der Personenseite kann man die Schüleranzahl des Teams eintragen.
Hierzu zählen auch Schüler, die keine eigenen Zugangsdaten für das Wettkampfsystem haben.
Diese Zahl wird sowohl für die Presse als auch für statistische Zwecke gebraucht
und sollte in jedem Fall stimmen.
...
In dieser Rubrik kann man die Computerspieler hochladen und verwalten. Bevor man
einen Computerspieler hochlädt, muss <<Computerspieler abgabefertig machen,der
Computerspieler abgabefertig gemacht werden>>.

image::computerspieler_uebersicht.jpg[Übersicht der eingesendeten Computerspieler]

Oben am rechten Rand befindet sich die Schaltfläche, um neue Computerspieler
hinzuzufügen. Diese führt zu einem Formular, mit dem man das ZIP-Archiv des
Computerspielers hochladen kann.

image::computerspieler_hochladen.jpg[Forumlar zum Einsenden eines Computerspielers]

Man kann dem Spieler einen *Namen* geben, damit man ihn in der Liste der
hochgeladenen Spieler später besser erkennt. Man kann auch noch zusätzliche
*Parameter* festlegen, die dem Computerspieler beim Start übergeben werden. Dies
ist nützlich, wenn der Computerspieler verschiedene Spielstrategien unterstützt
und man diese per Parameter auswählen kann. Dann muss man den Computerspieler
nur einmal hochladen (die Parameter kann man auch später verändern). Die Angabe
eines Namens und von Parametern ist optional.

Als letztes muss noch die Umgebung (das *Docker Image*) gewählt werden, in der der
Computerspieler auf dem Wettkampfsystem ausgeführt werden soll. Verwendet der
Computerspieler eine der beiden offiziell unterstützten Programmiersprachen Java
und Ruby, kann hier einfach das entsprechende ausgewählt werden. Ansonsten hängt
es von der gewählten Programmiersprache ab, ob eine passende Umgebung angeboten
wird. Sollte sich nichts finden, installieren wir gern etwas passendes nach.
